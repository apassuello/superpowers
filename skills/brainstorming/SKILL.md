...
- **Bounded** — a well-scoped change to code that already exists in
  ...
  one. No spec file, no implementation plan document — but the design
  you present in chat still pins behavior to exact values: literal
  inputs and outputs, error messages, boundary results, for everything
  the change touches. The artifact gets lighter; the contract does not
  get looser. If you cannot state the behavior exactly in chat, you do
  not understand the change well enough to call it bounded — step up
  to Architectural.
